...
class DivideComplex(Scene):
  def construct(self):
    ...
    self.wait()
    denom_angle = Angle(xaxis, denom_vec, color = ORANGE, radius=1)
    self.play(Create(denom_angle))
    self.wait()
    denom_angle_measure = Integer(denom_angle.get_value(degrees = True), unit = "^{\circ}", color=ORANGE).next_to(denom_angle)
    self.play(Write(denom_angle_measure))
    self.wait()
    denom_brace = BraceBetweenPoints(plane.c2p(2,1), plane.c2p(0,0), color=ORANGE)
    self.play(Create(denom_brace))
    denom_mag = denom_brace.get_text(r"$\sqrt{5}$").set_color(ORANGE)
    self.play(Write(denom_mag))
    self.wait()
    denom_mag_and_angle = Tex(r"$\sqrt{5} \quad \,\, 27^\circ$").set_color(ORANGE).next_to(numer_mag_and_angle, 1.5*DOWN)
    self.play(Write(denom_mag_and_angle))
    self.wait()
    denom_grp = VGroup(denom_box, denom_vec, denom_angle, denom_angle_measure, denom_brace, denom_mag)
    self.play(Uncreate(denom_grp))
    self.wait()
    answer_box = SurroundingRectangle(problem2_complete[0][8:], color=BLUE)
    self.play(Create(answer_box))
    self.wait()
    answer_vec = Line(start = plane.c2p(0,0), end=plane.c2p(1,-1), color=BLUE).add_tip()
    self.play(Create(answer_vec))
    self.wait()
    answer_angle = Angle(xaxis, answer_vec, color=BLUE, radius=0.8)
    self.play(Create(answer_angle))
    self.wait()
    answer_angle_measure = Integer(answer_angle.get_value(degrees = True), unit = "^{\circ}", color=BLUE).next_to(answer_angle, UL)
    self.play(Write(answer_angle_measure))
    self.wait()
    answer_brace = BraceBetweenPoints(plane.c2p(1,-1), plane.c2p(0,0), color=BLUE)
    self.play(Create(answer_brace))
    answer_mag = answer_brace.get_text(r"$\sqrt{2}$").set_color(BLUE)
    self.play(Write(answer_mag))
    self.wait()
    answer_mag_and_angle = Tex(r"$\sqrt{2} \quad 315^\circ$").set_color(BLUE).next_to(denom_mag_and_angle, 1.5*DOWN)
    self.play(Write(answer_mag_and_angle))
    answer_grp = VGroup(answer_box, answer_vec, answer_angle, answer_angle_measure, answer_brace, answer_mag)
    self.play(Uncreate(answer_grp))
    self.wait()