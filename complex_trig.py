...
class ComplexMult(Scene):
    def construct(self):
        ...
        angleA_lbl = Tex(r"$A$").next_to(angleA, 0.25*RIGHT + 0.05*UP)
        ...
        self.wait()
        dot3_lbl = Tex(r"$\cos(A+B) + i\sin(A+B)$").next_to(dot3).set_color(BLUE).scale(0.8)
        ...
        self.wait()
        problem2 = Tex(r"$\cos(A)\cos(B) + \cos(A)\sin(B)i + \sin(A)\cos(B)i + \sin(A)\sin(B)i^2$").next_to(problem1, DOWN).scale(0.9)
        self.play(Write(problem2))
        self.wait()
        i2_box = SurroundingRectangle(problem2[:-2], color=YELLOW)
